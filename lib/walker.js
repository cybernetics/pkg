...
class Walker {
  appendSub (task) {
    const { file, tuple } = task;
    assert(!this.records[file]);
    this.records[file] = { file, tuple };
...
*/
  ...
  async step_STORE_STAT (record) { // eslint-disable-line camelcase
    ...
    record[STORE_STAT] = { directly: true };
  ...
  async step (task) {
    ...
    if (task.store === STORE_CODE ||
      ...
      await this.step_STORE_ANY(record, task.store);
    } else
    if (task.store === STORE_LINKS) {
      this.step_STORE_LINKS(record, task.data);
    } else
    if (task.store === STORE_STAT) {
      await this.step_STORE_STAT(record);
    } else {
      assert(false, 'walker: unknown store ' + task.store);
    }
  }
