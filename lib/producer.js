import { STORE_CODE, STORE_CONTENT, snapshotify } from '../prelude/common.js';
      ...
      if (count === 5) {
        ...
        if (task.buffer) {
          if (task.store === STORE_CODE) {
            const snap = snapshotify(task.file, slash);
            return compile(options, target, snap, task.buffer, (error, buffer) => {
